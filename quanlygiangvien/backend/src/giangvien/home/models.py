...
class Instructor(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        if not self.user:
            # Tạo tài khoản người dùng mới với username là instructorID và mật khẩu mặc định là 1111
            user = User.objects.create_user(username=self.instructorID, email=self.email, password='1111')
            self.user = user
